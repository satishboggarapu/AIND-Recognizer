   ...
   """
    ...
    for testing_word, (X, lengths) in test_set.get_all_Xlengths().items():
        ...
        for trained_word, model in models.items():
            try:
                log_probaility = model.score(X, lengths)
                probability_dict[trained_word] = log_probaility
            except:
                probability_dict[trained_word] = float("-inf")

            if log_probaility > best_score:
                best_score, best_guess = log_probaility, trained_word
